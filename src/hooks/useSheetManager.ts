...
function generateId(): string {
  return 'character_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7)
}
...
export function useSheetManager() {
  ...
  const carregarRegistro = useCallback(async () => {
    ...
    try {
      ...
      newRegistry.sort((a, b) => new Date(b.atualizadoEm).getTime() - new Date(a.atualizadoEm).getTime())
  ...
  const criarFicha = useCallback(async (tipo: SheetTipo = 'completa'): Promise<string> => {
    const id = generateId()
  ...
  const importarFicha = useCallback(async (data: Partial<Character>): Promise<string> => {
    const id = generateId()
  ...
  const removerFicha = useCallback(async (id: string) => {
  ...
  const duplicarFicha = useCallback((id: string): string | null => {
    const entry = registry.find(r => r.id === id)
    if (!entry) return null

    const novoId = generateId()

    characterRepo.findById(id).then(original => {
  ...
  const fichasCompletas   = registry.filter(f => f.tipo === 'completa')
  const fichasNpc        = registry.filter(f => f.tipo === 'npc')
  const fichasSimplificadas = registry.filter(f => f.tipo === 'simplificada')
