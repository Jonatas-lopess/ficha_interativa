...
export function useCharacter(
  ...
  const defaultChar = (): Persisted<Character> => ({
  ...
  useEffect(() => {
    ...
    characterRepo
      ...
      .catch((e) => {
        console.warn(
          "Ficha ainda não propagada ou não existe, usando default.",
          e.message,
        );
  ...
  const updateNestedField = useCallback(
    <K extends keyof Character, NK extends keyof Character[K]>(
      parent: K,
      field: NK,
      value: Character[K][NK],
    ) => {
  ...
  const updateRanque = useCallback(
    (novoRanque: RanqueNome) => {
      ...
      setCharacter((prev) => {
        ...
        const novoEstresse: EstresseEstado[] =
          novoTamanho > estresseAtual.length
            ? [
                ...estresseAtual,
                ...Array(novoTamanho - estresseAtual.length).fill("livre"),
              ]
        ...
        const next = {
          ...
          divino: {
            ...prev.divino,
            ativo: novoRanque !== "Humano" ? prev.divino.ativo : false,
          },
        };
        handleUpdate(next);
        return next;
  ...
  const toggleEstresse = useCallback(
    (index: number) => {
      setCharacter((prev) => {
  ...
  const adjustEstresse = useCallback(
    (delta: number) => {
      setCharacter((prev) => {
        const novoEstresse = [...prev.estresse];
        if (delta > 0) {
  ...
  const rankData: RankData =
    RANQUES[character?.ranque as RanqueNome] || RANQUES.Humano;
